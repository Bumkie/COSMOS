...
class Planner:
    ...
    def func_yolo(self):
        ...
        while not self.stop_event.is_set():
            ...
            for detection_info in object_coor:
                ...
                if not(name == 'person' or name == 'tv' or name == 'banana'):
                    continue
    ...
    def func_mission(self):
        ...
        #2. 전방 이동
        self.socket8889.sendto('go 100 0 0 0'.encode('utf-8'), self.tello_address)   
        ...
        if not is_search:
            ...
            print(">>> [Mission] 복귀 성공")
            print(">>> [Mission] 종료")
            self.main.virtual_controller.onClose()
            self.btn_mission = False 
            return
    ...
    def pid_control(self, down_op, error_rate):
        ...
        while not self.stop_event.is_set():
            ...
            if person_coor is None and tv_coor is None:
                ...
                # 잘못 인식한 횟수가 6번 이상이면
                if x_cnt > 10:
                    print(">>> [Robomaster Tello Talent] person 인식에 10회 이상 실패했습니다.")
    ...
    def pid_control2(self, error_rate):
        ...
        while not self.stop_event.is_set():
            ...
            if person_coor is None and tv_coor is None:
                ...
                #짝수일 때는 하강
                if x_cnt%2 == 0:
                    self.socket8889.sendto('rc 0 0 -20 0'.encode('utf-8'), self.tello_address)
                    time.sleep(0.5)
                    self.socket8889.sendto('rc 0 0 0 0'.encode('utf-8'), self.tello_address)
                
                #홀수일 때는 상승
                else:
                    self.socket8889.sendto('rc 0 0 40 0'.encode('utf-8'), self.tello_address)
                    time.sleep(0.5)
                    self.socket8889.sendto('rc 0 0 0 0'.encode('utf-8'), self.tello_address)
                ...
                # 잘못 인식한 횟수가 10번 이상이면
                if x_cnt > 10:
                    print(">>> [Robomaster Tello Talent] 착륙장 인식에 10회 이상 실패했습니다.")
            ...
            pixel_x = x2-x1
            pixel_y = y2-y1
            h = 0.5*((self.frame_width*228)/pixel_x + (self.frame_height*228)/pixel_y)
